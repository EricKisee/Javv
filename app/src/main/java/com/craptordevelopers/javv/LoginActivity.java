...
public class LoginActivity extends AppCompatActivity implements LoaderCallbacks<Cursor> {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        user = new User( getApplicationContext() , this);
        try{
            wait(5000);
        }catch (Exception e){

        }
        if (user.isUser_is_signed_in()){
            // User is signed in
    ...
    private void signin (String email , String password){
        if(user.signinUser(email,password)){
            //success
            Toast.makeText(LoginActivity.this,"User Logged in "+user.getEmail(),Toast.LENGTH_SHORT).show();
            Log.d(TAG, "onAuthStateChanged:signed_in:" + user.getEmail());
        }else{
    ...
    private void attemptLogin() {
        ...
        if (cancel) {
            ...
            // Show a progress spinner, and kick off a profile_cover_default task to
